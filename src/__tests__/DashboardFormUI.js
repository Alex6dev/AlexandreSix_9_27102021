...
const bills = {
  ...
  "commentary": "séminaire billed",
  "name": "encore",
  "fileName": "preview-facture-free-201801-pdf-1.pdf",
  "date": "2004-04-04",
  "amount": 400,
  "email": "a@a",
  "pct": 20
...
describe('Given I am connected as an Admin and I am on Dashboard Page', () => {
  describe('When bill data is passed to DashboardUI', () => {
    test(('Then, it should them in the page'), () => {
      const html = DashboardFormUI(bills) 
      document.body.innerHTML = html
      expect(screen.getByText(bills.vat)).toBeTruthy()
      expect(screen.getByText(bills.type)).toBeTruthy()
      expect(screen.getByText(bills.commentary)).toBeTruthy()
      expect(screen.getByText(bills.name)).toBeTruthy()
      expect(screen.getByText(bills.fileName)).toBeTruthy()
      expect(screen.getByText(formatDate(bills.date))).toBeTruthy()
      expect(screen.getByText(bills.amount.toString())).toBeTruthy()
      expect(screen.getByText(bills.pct.toString())).toBeTruthy()
    })
  })
  describe('When pending bill is passed to DashboardUI', () => {
    test(('Then, it should show button and textArea'), () => { 
      const html = DashboardFormUI(billsPending)
      document.body.innerHTML = html
      expect(screen.getByText("Accepter")).toBeTruthy()
      expect(screen.getByText("Refuser")).toBeTruthy()
      expect(screen.getByTestId("commentary2")).toBeTruthy()
  ...
  describe('When accepted bill is passed to DashboardUI', () => {
    test(('Then, it should show admin commentary'), () => {
      const html = DashboardFormUI(billsAccepted)
      document.body.innerHTML = html
      expect(screen.getByText(bills.commentAdmin)).toBeTruthy()
    })
  })
  describe('When acceptrefuseded bill is passed to DashboardUI', () => {
    test(('Then, it should show admin commentary'), () => {
      const html = DashboardFormUI(billsrefused)
      document.body.innerHTML = html
      expect(screen.getByText(bills.commentAdmin)).toBeTruthy()
    })
  })
